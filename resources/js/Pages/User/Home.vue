...
const filters = ref({
    ...
    type: "", // "full-time", "part-time", or ""
    salary: 0, // Minimum salary filter
    name: "", // Filter by internship name
    city: "", // Filter by city
});

// Computed property to filter internships
const filteredInternships = computed(() => {
    return props.internships.filter((internship) => {
        const statusMatch =
            !filters.value.status ||
            (filters.value.status === "open" && internship.is_open) ||
            (filters.value.status === "closed" && !internship.is_open);

        const typeMatch =
            !filters.value.type ||
            filters.value.type === internship.employment_type;

        const salaryMatch =
            !filters.value.salary || internship.salary >= filters.value.salary;
        ...
        return (
            statusMatch && typeMatch && salaryMatch && nameMatch && cityMatch
        );
    });
});
...
<template>
    <div class="flex max-w-6xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        <div class="flex flex-col gap-6">
            <!-- Filter Section -->
            <div class="flex flex-wrap items-center gap-4 p-4 pl-12">
                <!-- Filter by Name -->
                <div>
                    <label
                        for="nameFilter"
                        class="block text-sm font-extrabold text-black"
                    >
                        Search by Name
                    </label>
                    <input
                        ...
                        placeholder="Search internship name"
                        class="w-full mt-1 px-4 py-2 text-gray-300 rounded-lg focus:outline-none focus:ring-[#00FFAB] focus:border-[#00FFAB]"
                    />
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Enter salary (e.g., 10000)"
                        class="w-full mt-1 px-4 py-2 text-gray-300 rounded-lg focus:outline-none focus:ring-[#00FFAB] focus:border-[#00FFAB]"
                    />
                </div>
            </div>
...
<ul class="space-y-4">
  <li
    ...
    :class="[
      'relative bg-white/90 rounded-xl shadow-lg hover:shadow-2xl transition-transform transform hover:scale-[1.03] cursor-pointer p-5 border border-gray-200 overflow-hidden',
      selectedInternship?.id === internship.id ? 'border-[#00FFAB] shadow-xl scale-[1.05]' : ''
    ]"
  >
    <!-- Ribbon for Status -->
    <span
      class="absolute top-0 left-0 text-black text-xs font-semibold px-3 py-1 rounded-br-xl shadow-md"
      :class="
        internship.is_open
      ...
      {{ internship.description }}
                ...
                <div class="w-full md:w-1/2 sticky top-24 h-fit">
                    <div
                        ...
                        class="p-8 bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200 hover:scale-[1.02] transform transition-transform max-w-full md:max-w-[500px]"
                    >
                        <!-- Internship Title -->
                        <h2
                            class="text-4xl font-extrabold text-gray-900 mb-4 leading-tight tracking-tight truncate"
                            title="{{ selectedInternship.internship_name }}"
                        >
                            {{ selectedInternship.internship_name }}
                        </h2>
                        <p
                            class="text-lg text-gray-600 flex items-center gap-2 mb-6 truncate"
                        >
                            <i class="fas fa-building text-[#00FFAB]"></i>
                        ...
                        <!-- Details -->
                        <p
                            class="text-gray-700 text-lg leading-relaxed mb-6 line-clamp-3 overflow-hidden"
                            title="{{ selectedInternship.about }}"
                        >
                        ...
                        >
                            ...
                            <div class="flex items-center gap-2">
                                <i
                                    class="fas fa-calendar-alt text-[#00FFAB]"
                                ></i>
                                <p>
                            ...
                            <div class="flex items-center gap-2">
                                <i
                                    class="fas fa-check-circle text-[#00FFAB]"
                                ></i>
                                <p>
                            ...
                            <div class="flex items-center gap-2">
                                <i
                                    class="fas fa-dollar-sign text-[#00FFAB]"
                                ></i>
                                <p>
                        ...
                        <div class="flex justify-center mt-8 gap-4">
                            ...
                            <button
                                ...
                                :disabled="
                                    ...
                                    !selectedInternship.is_open
                                "
                                class="px-6 py-2 text-black font-bold rounded-xl shadow-md transition-transform hover:scale-105"
                                :class="{
                                    'bg-green-500 hover:bg-green-600':
                                        selectedInternship?.is_open,
                                    'bg-red-500 text-white disabled:cursor-not-allowed':
                                        !selectedInternship?.is_open,
                                }"
                            >
                                {{
                        ...
                        >
                            {{
                                selectedInternship?.is_open ? "Open" : "Closed"
                            }}
                        </span>
                    </div>
                    ...
                    <div
                        v-else
                        class="flex flex-col items-center justify-center text-center text-gray-500 py-24 bg-white/80 backdrop-blur-md rounded-2xl shadow-xl max-w-full md:max-w-[500px]"
                    >
                        <i
        ...
        >
            ...
            <div class="mb-6">
                ...
                >
                    Apply for
                    <span class="text-[#00FFAB] font-extrabold">{{
                        selectedInternship.internship_name
                    }}</span>
                </h1>
                <p
                ...
                >
                    Provide your resume and cover letter to submit your
                    application.
                </p>
            </div>
            ...
            >
                ...
                <div class="mb-6">
                    ...
                    >
                        Selected File:
                        {{ applicationForm.documents.resume.name }}
                    </p>
                    <!-- Error Message -->
                    <p
                ...
                <div class="mb-6">
                    ...
                    >
                        Selected File:
                        {{ applicationForm.documents.cover_letter.name }}
                    </p>
                    <!-- Error Message -->
                    <p
                ...
                <div class="flex justify-end space-x-4 mt-8">
                    ...
                    >
                        <span v-if="applicationForm.processing"
                            >Submitting...</span
                        >
